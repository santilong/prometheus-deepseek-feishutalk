...
from Prom_Feishu import Feishu_Alert, Grafana_Data, HttpClient, local_config
...
class AlertProcessor(Thread):
    ...
    def run(self):
        while processing_thread_active:
            try:
    ...
    {f"当前值: {annotations.get('value', 'Unknown')}" if annotations.get('value') else ''}"""
        ...
        with httpx.Client(verify=False) as client:
            response = client.post(
                url=self.feishu.webhook, json=message, headers=headers, timeout=10
            )
