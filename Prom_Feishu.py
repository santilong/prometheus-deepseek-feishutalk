...
class HttpClient:
    ...
    def http_get(self, url, params=None, headers=None):
        ...
        raise httpx.RequestError(
            f"请求 {url} 失败，已达最大重试次数 {self.max_retries}"
        )
    ...
    def Get_Data(self):
        ...
        try:
            ...
            response = client.http_get(
                url=pro_url, params={}, headers={"Content-Type": "application/json"}
            )
            # print(f"请求成功, 状态码: {response.status_code}")
            if response.status_code == httpx.codes.OK:
                config = {
                    "query": json.dumps(
                        response.json()["data"]["groups"][0]["rules"][0]["query"],
                        indent=2,
                    ),
                    "alert_name": json.dumps(
                        response.json()["data"]["groups"][0]["rules"][0]["name"],
                        indent=2,
                    ),
                    "duration": json.dumps(
                        response.json()["data"]["groups"][0]["rules"][0]["duration"],
                        indent=2,
                    ),
                }
                # print(config)
    ...
    def Get_Dashboard(self, panel_id):
        ...
        try:
            ...
            if self.panel_id is not None:
                ...
                render_url = f"{self.address}/render/d-solo/{self.dashboard_id}/?orgId=1&panelId={self.panel_id}&width=1000&height=500"
                response = client.http_get(url=self.url, headers=self.headers)

                img = client.http_get(url=render_url, headers=self.headers)
...
class Feishu_Alert:
    ...
    def get_ImageKey(self):
        ...
        with open(self.file_path, "rb") as file:
            ...
            response = client.post(
                url=get_image_key_url, headers=image_key_headers, data=data, files=files
            )  # 相应内容: {'code': 0, 'data': {'image_key': 'img_v3_02j2_1ec9ed69-730c-42a1-91e4-4e3e5cf867cg'}, 'msg': 'success'}
